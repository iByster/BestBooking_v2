import { EskyRoWorkerPayload, EskyRoWorkerResponse } from "../../../types/types";
...
parentPort.on('message', async (payload: EskyRoWorkerPayload) => {
    ...
    let response: EskyRoWorkerResponse = { workerId: threadId, data: null, error: null };
    
    await randomDelay(30000, 35000);
    
    try {
        const workerResponse = await scrapeHotelByUserInput(hotelUrl, userInput, cookie, parseInt(siteHotelId), existingHotel);
